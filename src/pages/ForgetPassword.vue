...
const emailHandler = async () => {
  ...
  loading.value = true;
  try {
    const { data } = await axios.post("/customer/auth/validate-email", { email: email.value, })
    if (data.success == true) {
      router.push({ name: "verify-otp", query: { email: email.value, reset_password: true } });
    } else {
      toast.error(data?.message || "Email Don't exist.")
    }
