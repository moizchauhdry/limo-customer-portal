...
const loginHandler = async () => {
  ...
  try {
    ...
    console.log(data);
    console.log(data.data?.account_not_verified);

    const token = data?.data?.token;
    if (data.data?.account_not_verified == true) {
      console.log(`account_not_verified: `, data.data?.account_not_verified);
  ...
  <body class="flex items-center bg-[#FFFFFF] justify-center">
    ...
    >
      ...
      <form class="p-8 space-y-5" @submit.prevent="loginHandler">
        ...
        <p
          class="text-lg text-[#414141] font-poppins font-normal text-center mb-6"
        >
        ...
        <p class="text-center text-md text-[#414141] font-poppins">
          Don’t have account?
          <router-link
            class="text-blue-600 hover:text-black underline"
            to="/auth/signup"
            >sign up</router-link
          >
        </p>
